...
⚠️ **Apply Loop measurement bug *(confirmed 2026-05-22 — third session)*:** `compute-apply-ratio.mjs` reports RED 25% even when uniqueness rewrites are actually done, because the script counts via `git log` keyword matches (rewrite/regenerate) — it does NOT recognize "content additions to existing pages that improve uniqueness." Real fix: the script should diff today's `uniqueness-audit.json` against yesterday's and credit any pages that moved below-floor → above-floor as "rewrites applied" regardless of commit message. Until this is fixed, the 25% RED verdict can be misleading — verify the actual uniqueness backlog count before trusting the verdict. Next session priority: patch the script.
...
- **Microsites (static HTML):** `CLOUDFLARE_API_TOKEN=$TOKEN CLOUDFLARE_ACCOUNT_ID=c0a563aca1a6032f711bdc29ebeccb95 npx wrangler pages deploy ~/Workspaces/<site> --project-name <project-name> --branch main`
  ...
  - **Workspace exists ≠ CF Pages project exists** *(added 2026-05-22)*. Sites added to `ACTIVE_SITES` in `score-active-sites.mjs` (and bucketed in `rotation-calendar.mjs`) may have a workspace folder with bail content but NO Cloudflare Pages project. Example: `hermosa-beach-bail-bonds` workspace exists with bail content dated 2026-04-14, but the CF Pages project was never created — all 5 project-name variants returned `code: 8000007 Project not found`. The blog gets committed to GitHub as deferred-value but cannot deploy live. Resolution before rotation activates: either (a) create CF Pages project via `wrangler pages project create <name>`, (b) confirm the site is hosted elsewhere (WordPress, Lovable, custom) and add to a "non-CF-deploy" exclusion list, or (c) remove from rotation. Tracked: `hermosa-beach-bail-bonds` 2026-05-22.
...
8.5. **Per-site change log + Apply Loop scorer + Strategy results** *(added 2026-05-13)*: run THREE scripts BEFORE the final report so Section #27, #28, and #29 have data to render.
   ...
   - `node ~/Workspaces/eros-workspace/scripts/check-dashboard-sync.mjs` *(added 2026-05-13)* — audits the EROS Dashboard for drift. Verifies: server reachable on :3737, /api/skills matches disk, every Phase has data-sinks + plain-language coverage, glossary covers acronyms, skill-audit covers every skill, referenced scripts exist on disk, today's data JSONs preview-able. Writes `data/daily/<date>/dashboard-sync.json` with verdict IN SYNC / DRIFTING / BROKEN. Exit code 1 on any FAIL check. Section #30 renders this in the final report.
   - `node ~/Workspaces/eros-workspace/scripts/surface-pending-strategies.mjs` *(added 2026-05-19)* — reads every `data/research/<date>/findings.md`, extracts 🔴 ADD items, filters out those already in `~/Workspaces/eros-dashboard/strategies.json` (by `source` field match). Writes `data/daily/<date>/pending-strategies.json` with full per-item detail + auto-suggested test design (window / treatment / control / signal / estimated cost / success+kill criteria). Section #28.5 renders this so Angelo can answer YES/NO/LATER on each pending strategy from the email. Solves the gap where research findings sit in `findings.md` for weeks without being lined up for testing.
   - `node ~/Workspaces/eros-workspace/scripts/strategy-closeout-report.mjs` *(added 2026-05-19)* — detects strategies whose `endDate ≤ today` and `status='active'`. For each closing strategy: computes explicit 0-100 score using weighted rubric (click volume 40 pts / impression reach 20 pts / position improvement 20 pts / stability 20 pts), assigns verdict (≥75 KEEP_SUCCESS / 41-74 NEUTRAL / ≤40 KILL_FAIL), generates detailed markdown closeout report including setup, daily history, score breakdown, recommendation, and next steps. Writes per-strategy report to `data/strategy-closeouts/<strategy-id>-<end-date>.md`. Writes summary index to `data/daily/<date>/closeouts.json`. Section #29.7 renders the summary table with link to each full report.
   - Run order matters: build-site-changelog FIRST, then compute-apply-ratio (reads the changelog as research-apply evidence), then measure-strategy-results, then build-strategy-timeline, then check-dashboard-sync, then surface-pending-strategies, then strategy-closeout-report (last — depends on all prior strategy data being on disk)
...
9. **MANDATORY — EROS Day Final Report** (added 2026-04-28, enriched 2026-04-29 + 2026-05-06): `node ~/Workspaces/eros-workspace/scripts/eros-day-final-report.mjs`
   ...
   - Sections (always all of them, in this order):
     ...
     28.5. **📋 Pending Strategy Decisions — Awaiting Your Call** *(added 2026-05-19)* — reads `data/daily/<date>/pending-strategies.json`. For each 🔴 ADD finding from past research that is NOT yet in `strategies.json`: title, source date + section, full detail block, EROS recommendation excerpt, AUTO-SUGGESTED test design (window / treatment scope / control scope / signal / estimated cost / success+kill criteria), and a YES/NO/LATER answer block. Footer explains how to respond ("reply to this email with `strategy: <title> = YES`"). Solves the gap where research findings sat in `findings.md` for weeks without being lined up for testing — now Angelo sees every pending strategy with full decision context inline in the daily email.
     ...
     29.7. **🏁 Strategy Closeout Reports — Tests Ending Today** *(added 2026-05-19)* — reads `data/daily/<date>/closeouts.json`. For each strategy whose `endDate ≤ today` and `status='active'`, renders: final 0-100 score, verdict (KEEP_SUCCESS / NEUTRAL / KILL_FAIL), one-line recommendation, link to the full per-strategy markdown report at `~/Workspaces/eros-workspace/data/strategy-closeouts/<id>-<date>.md`. Section closer: scoring rubric reference (40 pts click volume / 20 pts impressions / 20 pts position / 20 pts stability) so Angelo can see how each score was earned. Every test produces an auditable, dated decision record — no more verbal "I think it worked" calls.
